...
export function filterGlossaryByPassage(pack: ContentPack): ContentPack {
  const kept = pack.glossary.filter((g) => pack.passage_ko.includes(g.surface));
  if (kept.length !== pack.glossary.length) {
    const dropped = pack.glossary.filter((g) => !pack.passage_ko.includes(g.surface)).map((g) => g.surface);
